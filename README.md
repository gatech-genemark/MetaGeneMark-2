...
Running MetaGeneMark-2 using automatic genetic code detection is done through the `run_mgm2.pl` script found in `src`.
...
Running MetaGeneMark-2 with automatic genetic code detection is done using `run_mgm2.pl`. The following files should be in the same directory: `run_mgm2.pl`, `gmhmmp2`, `mgm2_11.mod`, `mgm2_4.mod`. MetaGeneMark-2 can then be run (from anywhere) using:

    $path_to_binary/run_mgm2.pl --seq [name]  --out [name]
